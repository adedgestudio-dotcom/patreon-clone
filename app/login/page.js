...
const Login = () => {
  ...
  React.useEffect(() => {
    if (session) {
      router.push("/dashboard"); // if user is logged in it will redirect to dashboard
    }
  }, [session, router]);
  return (
    <div className="text-[var(--color-text)] container mx-auto py-14">
      ...
      <div className="flex flex-col gap-2 min-h-screen items-center p-10">
        ...
        <button
          ...
          className="flex items-center w-64 bg-slate-50 text-black border border-gray-300 rounded-lg shadow-md max-w-xs px-6 py-2 text-sm font-medium  hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500"
        >
          <svg
            className="h-6 w-6 mr-2"
            xmlns="http://www.w3.org/2000/svg"
